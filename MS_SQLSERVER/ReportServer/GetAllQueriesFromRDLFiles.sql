...
/* The XML XQuery statements used below ignore namespace so that information can be retrieved
   ...
   --from https://bengribaudo.com/blog/2014/08/07/3278/ssrs-auditing-report-queries */
USE ReportServer;
...
AS (
	...
	SELECT r.ItemID
		...
		, -- embedded data source's name or local name given to shared data source
		sds.SharedDataSourceName
		, SharedDataSource = CAST((
				CASE 
	...
	LEFT JOIN SharedDataSources sds
...
AS (
	...
	SELECT ds.ItemID
		...
		, -- "Text" = default command type
		CommandText
	FROM DataSets ds
	INNER JOIN AllDataSources src
