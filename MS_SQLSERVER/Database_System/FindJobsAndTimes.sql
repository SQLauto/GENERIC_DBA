SELECT S.name AS JobName
	...
	, CASE (SS.freq_type)
		...
		WHEN 32
			THEN 'Every ' + convert(VARCHAR(3), SS.freq_recurrence_factor) + ' Months' -- RELATIVE
		WHEN 64
			THEN 'SQL Startup'
		WHEN 128
			THEN 'SQL Idle'
	...
	, CASE (freq_subday_type)
		WHEN 1
			THEN left(stuff((
							stuff((replicate('0', 6 - len(active_start_time))) + convert(
									VARCHAR(6), active_start_time), 3, 0, ':')
							), 6, 0, ':'), 8)
		WHEN 2
	...
	, CASE SJ.next_run_date
		...
		ELSE convert(CHAR(10), convert(DATETIME, convert(CHAR(8), SJ.next_run_date)), 120) + ' ' + left(stuff((
						stuff((replicate('0', 6 - len(next_run_time))) + convert(VARCHAR(6), 
								next_run_time), 3, 0, ':')
						), 6, 0, ':'), 8)
		END AS NextRunTime
